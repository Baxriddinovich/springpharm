<?php
// sections.php - GMP Bo'limlarini boshqarish (Bosh Auditor uchun moslashtirilgan)
require_once '../db.php';
requireLogin();
...
// ⭐ Role-based access (Admin va Bosh Auditor uchun ruxsat)
if (!in_array($user['role'], ['super_admin', 'bosh_auditor', 'admin'])) {
    $_SESSION['flash_message'] = "Bu sahifaga kirish uchun ruxsat yo'q!";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!verifyCsrf()) {
        ...
        if ($action === 'add_section') {
            ...
            if ($number !== '' && $name !== '') {
                ...
                if ($dupCheck->fetch()) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO gmp_sections (section_number, section_name, description, sort_order) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$number, $name, $desc, $order]);
                    logActivity('section_added', "Yangi bo'lim qo'shildi: $number - $name");
        ...
        if ($action === 'edit_section') {
            ...
            if ($id > 0 && $number !== '' && $name !== '') {
                ...
                if ($dupCheck->fetch()) {
                    ...
                    $stmt = $pdo->prepare("UPDATE gmp_sections SET section_number = ?, section_name = ?, description = ?, sort_order = ? WHERE id = ?");
                    $stmt->execute([$number, $name, $desc, $order, $id]);
                    logActivity('section_edited', "Bo'lim tahrirlandi: $number - $name (ID: $id)");
        ...
        if ($action === 'delete_section') {
            ...
            if ($id > 0) {
                $check = $pdo->prepare("SELECT COUNT(*) FROM checklist_questions WHERE section_id = ?");
                $check->execute([$id]);
                $questionCount = $check->fetchColumn();

                if ($questionCount > 0) {
                    $error = "Bu bo'limda $questionCount ta savollar mavjud! Avval savollarni o'chiring.";
                    $messageType = 'danger';
                } else {
                    ...
                    $stmt = $pdo->prepare("DELETE FROM gmp_sections WHERE id = ?");
                    $stmt->execute([$id]);

                    if ($secInfo) {
                        logActivity('section_deleted', "Bo'lim o'chirildi: {$secInfo['section_number']} - {$secInfo['section_name']}");
                    }
                    $_SESSION['flash_message'] = "Bo'lim muvaffaqiyatli o'chirildi!";
                    $_SESSION['flash_type'] = 'success';
                    header("Location: sections.php");
                    exit;
                }
...
<head>
    ...
    <style>
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .sort-btn { opacity: 0; transition: opacity 0.2s ease; }
...
<body class="min-h-screen text-slate-100">
            ...
            <div class="p-6 h-full flex flex-col">
                ...
                <nav class="space-y-1 flex-1 overflow-y-auto">
                    ...
                    <div class="pt-4 mt-4 border-t border-slate-700/50">
                        ...
                        <a href="sectionss.php" class="nav-item active flex items-center gap-3 px-4 py-3 rounded-lg text-white" aria-current="page">
        ...
        <main class="flex-1 lg:ml-64 w-full">
            ...
            <div class="p-4 lg:p-8">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    ...
                    <div class="stat-card rounded-2xl p-5 animate-in col-span-2 lg:col-span-1">
                ...
                <div class="stat-card rounded-2xl overflow-hidden animate-in">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-slate-700/30" id="sectionsTable">
                                ...
                                <tr class="section-row" data-id="<?php echo $sec['id']; ?>">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        <a href="checklists.php?section=<?php echo $sec['id']; ?>" class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/25 transition-colors">
                                            <?php echo $sec['question_count']; ?> ta
                                        </a>
                                        <?php else: ?>
                                        <span class="text-slate-500 text-sm">0 ta</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-end gap-2">
                                            <button onclick='openModal("edit", <?php echo htmlspecialchars(json_encode($sec, JSON_UNESCAPED_UNICODE)); ?>)' class="p-2 rounded-lg text-cyan-400 hover:bg-cyan-500/10 transition-colors" title="Tahrirlash">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            </button>
                                            
                                            <?php if ($sec['question_count'] == 0): ?>
                                            <button onclick="confirmDelete(<?php echo $sec['id']; ?>, '<?php echo htmlspecialchars(addslashes($sec['section_number'])); ?>')" class="p-2 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors" title="O'chirish">
                ...
                <div class="stat-card rounded-2xl p-12 text-center animate-in">
                    ...
                    <p class="text-slate-500 mb-6">GMP talablarini boshqarish uchun bo'limlarni yarating.</p>
                ...
                <form method="POST" action="" id="sectionForm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="section_number" id="formNumber" required class="input-field w-full px-4 py-2.5 rounded-xl" placeholder="Masalan: I yoki 1" maxlength="10">
                        </div>
                        <div>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div class="modal-content relative w-full max-w-sm rounded-2xl p-6 text-center">
                ...
                <p id="deleteSectionName" class="text-cyan-400 font-medium mb-4"></p>
    ...
    <script>
        ...
        document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { closeModal(); hideDeleteModal(); } });
    </script>
</body>
</html>