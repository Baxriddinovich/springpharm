...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!verifyCsrf()) {
        ...
        if ($action === 'delete') {
            $id = (int)($_POST['question_id'] ?? 0);
            if ($id > 0) {
                $usageCheck = $pdo->prepare("SELECT COUNT(*) FROM audit_answers WHERE question_id = ?");
                ...
                logActivity('question_deleted', "Savol o'chirildi (ID: $id, $usageCount marta ishlatilgan)");
                $_SESSION['flash_message'] = "Savol muvaffaqiyatli o'chirildi!" . ($usageCount > 0 ? " (Eslatma: ushbu savol $usageCount ta auditda ishlatilgan edi)" : "");
                $_SESSION['flash_type'] = 'success';
            ...
            <div class="p-6 h-full flex flex-col">
                ...
                <nav class="space-y-1 flex-1 overflow-y-auto">
                    ...
                    <div class="pt-4 mt-4 border-t border-slate-700/50">
                        ...
                        <a href="checklists.php" class="nav-item active flex items-center gap-3 px-4 py-3 rounded-lg text-white" aria-current="page">
                ...
                <div class="border-t border-slate-700/50 pt-4 mt-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-slate-500"><?php echo ucfirst(str_replace('_', ' ', $user['role'])); ?><?php if (!$canManage) echo ' (Ko\'rish)'; ?></p>
                        </div>
                ...
                <div class="space-y-3 animate-in delay-3">
                    ...
                    <div class="stat-card rounded-2xl p-12 text-center">
                        <div class="w-20 h-20 rounded-full bg-slate-800/80 flex items-center justify-center mx-auto mb-6">
                            <svg class="w-10 h-10 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>
                        </div>
                        <h3 class="text-xl font-semibold text-white mb-2">Savollar topilmadi</h3>
                        <p class="text-slate-500 mb-6 max-w-md mx-auto">
                            <?php echo $selectedSection ? 'Tanlangan bo\'limda hali savollar qo\'shilmagan.' : 'Hali hech qanday savollar qo\'shilmagan.'; ?>
                        </p>
                        <?php if ($canManage): ?>
                    ...
                    <?php else: ?>
                        ...
                        foreach ($questions as $q): 
                            $qIndex++;
                            // Bo'lim sarlavhasini chiqarish (agar filter yo'q bo'lsa)
                        ...
                        <div class="flex items-center gap-3 mt-8 mb-4 <?php echo $qIndex === 1 ? 'mt-0' : ''; ?>">
                            <a href="?section=<?php echo $q['section_id']; ?>" class="w-9 h-9 rounded-lg bg-cyan-500/15 border border-cyan-500/20 flex items-center justify-center text-cyan-400 font-mono text-sm font-bold hover:bg-cyan-500/25 transition-colors" title="Faqat shu bo'limni ko'rish">
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                                        ...
                                        <span class="px-2 py-0.5 rounded-full bg-red-500/15 text-red-300 border border-red-500/20 flex items-center gap-1">
                                        ...
                                        <?php else: ?>
                                        <span class="px-2 py-0.5 rounded-full bg-slate-500/15 text-slate-500 border border-slate-500/20">
                                            Ixtiyoriy
                                        </span>
                                        <?php endif; ?>
                                        
                                        <span class="text-slate-600">
                                            Tartib: <?php echo $q['sort_order']; ?>
                                        </span>
                                    </div>
                                </div>
                        ...
                        <div class="text-center text-slate-600 text-sm mt-8 pt-4 border-t border-slate-800/50">
                            Jami ko'rsatilgan: <?php echo count($questions); ?> ta savol
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <form method="POST" action="" id="questionForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" id="questionOrder" min="0" value="0" class="input-field w-full px-4 py-2.5 rounded-xl">
                        </div>
                    ...
                    <div class="flex gap-3 mt-6">
                        <button type="button" onclick="closeModal()" class="flex-1 py-2.5 rounded-xl border border-slate-600 text-slate-300 hover:bg-slate-700/50 transition-colors font-medium">
                            Bekor qilish
                        </button>
    ...
    <div id="deleteModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div class="modal-content relative w-full max-w-sm rounded-2xl p-6 text-center">
                ...
                <p class="text-slate-400 mb-2">Ushbu savolni o'chirishga ishonchingiz komilmi?</p>
                <p class="text-xs text-slate-600 mb-6 bg-slate-800/50 p-2 rounded-lg">Diqqat: Savol faqatgina checklistdan yashiriladi. Avval bergan javoblar saqlanib qoladi.</p>
                ...
                <form method="POST" action="" id="deleteForm">
                    ...
                    <div class="flex gap-3">
                        <button type="button" onclick="closeDeleteModal()" class="flex-1 py-2.5 rounded-xl border border-slate-600 text-slate-300 hover:bg-slate-700/50 transition-colors">
                            Bekor qilish
                        </button>
                        <button type="submit" class="flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-medium transition-colors">
                            O'chirish
                        </button>
                    </div>
                </form>
    ...
    <script>
        ...
        function openModal(mode, data = null) {
            ...
            if (mode === 'edit' && data) {
                ...
                document.getElementById('questionOrder').value = data.sort_order;
                ...
                if (urlSection) {
                    document.getElementById('sectionId').value = urlSection;
                }
            }
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.ctrlKey && e.key === 'n' && <?php echo $canManage ? 'true' : 'false'; ?>) {
                e.preventDefault();
                openModal('add');
            }
        });
